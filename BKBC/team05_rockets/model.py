...
MODELS = {
    "XGBoost": XGBClassifier(
        n_estimators     = 150,
        max_depth        = 7,        # shallow trees reduce overfitting
        learning_rate    = 0.4,
        subsample        = 0.8,
    ...
    "TopK XGBoost": Pipeline([
        # Step 1: Select top 100 proteins by ANOVA F-score
        # This happens INSIDE each CV fold = no data leakage
        ("selector", SelectKBest(
            score_func = f_classif,  # ranks proteins by ATI separation
            k          = 100         # keep top 100 proteins
        )),
        # Step 2: XGBoost on the selected proteins
        ("clf", XGBClassifier(
            n_estimators     = 100,
            max_depth        = 4,
            learning_rate    = 0.05,
            subsample        = 0.8,
            colsample_bytree = 0.6,   # higher since we pre-selected features
            eval_metric      = "logloss",
            random_state     = RANDOM_SEED,
            n_jobs           = nslots,
